...
var validate = function (field, rule) {
    ...
    //错误对象
    var $error = get(vm.$verify.$errors, field);
...
Verify.prototype.check = function (group) {
    ...
    return verifyQueue.map(function (item) {
            if (get(rules, item)) {
                set(vm.$verify.$errors, item, []);
                return validate.call(self.vm, item, get(rules, item));
            }
        }).indexOf(false) === -1;
    ...
    Vue.directive("verify", {
        bind: function (el, binding, vnode, oldVnode) {
            ...
            el.addEventListener("focus", function () {
                set(vm.$verify.$errors, expression, []);
            });
            ...
            if (options && options.blur) {
                el.addEventListener("blur", function () {
                    vm.$verify.$errorArray = [];
                    validate.call(vm, expression, get(vm.$options.verify, expression));
                });
            }
    ...
    Vue.directive("remind", {
        update: function (el, binding, vnode, oldVnode) {
            ...
            if (vnode.context.$verify.$errors) {
                errorText = get(vnode.context.$verify.$errors, expression);
            }
            if (errorText.length) {
                domTools.apply(el, true);
                if (binding.modifiers.join) {
